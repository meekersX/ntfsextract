...
def parse_entry (mm):
    ...
    try:
        ##if fname[1][1] == 'Screenshots':
        print '{};{};{};{};{};{};{};{};{};{};{}'.format((mm.tell()-1024)/512,fname[1],stdinfo[1],stdinfo[2],stdinfo[3],stdinfo[4],fname[2],fname[3],fname[4],fname[5],data)
    except: pass
...
def load_and_start(filename, showstats):
    ...
    try:
        with open(filename, "rb") as f:
            ...
            print "sector;filenames;STD created;STD modified;STD Meta modified;STD accessed;FN created;FN modified;FN Meta modified;FN accessed"

            while 1:

                # locate (next) FILE entry
                ##file_pos_pointer = mm.find('\x46\x49\x4C\x45',file_pos_pointer)
                while mm.tell() < 438630400*512 and mm.read(4) != '\x46\x49\x4C\x45':
                    mm.seek(mm.tell() + 1020)

                if mm.tell() >= 438630400*512:
                    break
